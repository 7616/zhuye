$(document).ready(function () {
 ...
 (function(){
    ...
    var text = content.innerHTML;  // text ?????
    var span = document.createElement("span"); // ??????? SPAN ???
    var n = document.createElement("a");  // ??????? A ???
    span.innerHTML = text.substring(0,len); // SPAN ?????????? text ??? len ?????
    n.innerHTML = text.length > len ? "...???" : ""; // ?????? A ??????????????????????? len????????..??????????????
    n.href = "javascript:void(0)"; // ???? A ?????????????????
    n.onclick = function(){ // ??? A ??????????溯??
    // ??? A ????????????..?????????? A ????????ɡ??????SPAN ?????????? DIV ?????????????????? len ?????
    if (n.innerHTML == "...???"){
      n.innerHTML = "????";
      span.innerHTML = text;
    ...
    content.appendChild(span); // ?? SPAN ????? DIV ??
    content.appendChild(n);   // ?? A ????? DIV ??
  })();
